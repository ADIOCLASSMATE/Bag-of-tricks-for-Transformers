"""
Baseline GPT + logit softcap ablation.

Same as baseline (naive architecture + strong training recipe), but adds
logit softcapping: logits = softcap * tanh(logits / softcap).

trick: logit-softcap — caps logit magnitudes via tanh, preventing extreme
values and stabilizing training. Default softcap=30.0.
"""
...
class Hyperparameters:
    ...
    logit_softcap = float(os.environ.get("LOGIT_SOFTCAP", 30.0))  # trick: logit-softcap
...
class GPT(nn.Module):
    def __init__(
        ...
        tie_embeddings: bool,
        rope_base: float,
        logit_softcap: float,  # trick: logit-softcap
    ):
        super().__init__()
        self.tie_embeddings = tie_embeddings
        self.logit_softcap = logit_softcap  # trick: logit-softcap
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.tie_embeddings:
            logits_proj = F.linear(x, self.tok_emb.weight)  # trick: logit-softcap
        else:
            if self.lm_head is None:
                raise RuntimeError("lm_head is required when tie_embeddings=False")
            logits_proj = self.lm_head(x)  # trick: logit-softcap
        logits = self.logit_softcap * torch.tanh(logits_proj / self.logit_softcap)  # trick: logit-softcap
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        logit_softcap=args.logit_softcap,  # trick: logit-softcap
    ).to(device).bfloat16()
