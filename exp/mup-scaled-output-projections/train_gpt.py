"""
Naive architecture GPT with strong training recipe + muP-scaled output projections trick.
...
Trick: muP-scaled output projections — scale non-zero-init output projections by 1 / sqrt(2L),
matching the parameter-golf combo recipe.
    ...
    def _init_weights(self) -> None:
        for name, module in self.named_modules():
            if isinstance(module, nn.Linear) and getattr(module, "_zero_init", False):
                ...
                continue
            if isinstance(module, nn.Linear) and (".proj." in name or name.endswith(".proj")):
                # trick: muP-scaled output projections — scale non-zero-init output
                # projections by 1 / sqrt(2L), matching the parameter-golf combo recipe.
                with torch.no_grad():
