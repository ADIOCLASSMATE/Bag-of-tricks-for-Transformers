"""
trick: shared-ve — Inject the token embedding into the value (V) path of early attention layers.

Core modification:
  - The token embedding x (before any block processing) is passed as an additional
    signal to the first 3 attention layers.
  - Each of those layers adds a learned fraction of the token embedding into V
    before attention:  v = v + 5 * ve_lambda * token_embed
  - ve_lambda is a per-layer scalar initialized to 0.0, so training starts
    identically to baseline.
  - The 5x multiplier gives ~5x effective LR on ve_lambda via gradient scaling.

Differences from the original modded-nanogpt ValueEmbed (2024-12-04_ValueEmbed by @KoszarskyB):
  - Original: uses a SEPARATE embedding table (vte), 12x dim size, chunked per layer.
  - This version: REUSES the existing tok_emb weight (zero extra parameters).
  - Original: convex combination  v = (1-lambda)*v + lambda*ve  with lambda init 0.5.
  - This version: additive injection  v = v + 5*lambda*token_embed  with lambda init 0.0.
  - Original: VE injected into ALL 12 layers.
  - This version: VE injected only into the first 3 (shallow) layers.
  - Original: includes x0 residual in each Block.
  - This version: no x0 residual (standard sequential residual only).

The design choice reflects a hypothesis: the raw token identity signal is most
valuable in shallow layers where local features dominate; deeper layers rely on
abstracted representations and do not benefit from raw token injection.
"""
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.ve_lambda = nn.Parameter(torch.tensor(0.0))
    ...
    def forward(self, x: Tensor, shared_token_embed: Tensor | None = None) -> Tensor:
        ...
        if shared_token_embed is not None:
            v = v + 5 * self.ve_lambda * shared_token_embed.view_as(v)
