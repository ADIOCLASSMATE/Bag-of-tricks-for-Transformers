"""
Baseline GPT + Q-Gain ablation.

Same as baseline (naive architecture + strong training recipe), but adds
learnable per-head Q-Gain parameter that scales query vectors after RoPE.

trick: Q-Gain — nn.Parameter(torch.full((num_heads,), 1.5)) applied as
q = q * q_gain[None, :, None, None] after QK-Norm + RoPE. Initialized to
1.5 to boost attention logit magnitudes.
"""
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.q_gain = nn.Parameter(torch.full((num_heads,), 1.5, dtype=torch.float32))  # trick: q-gain
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        q = q * self.q_gain.to(dtype=q.dtype)[None, :, None, None]  # trick: q-gain
