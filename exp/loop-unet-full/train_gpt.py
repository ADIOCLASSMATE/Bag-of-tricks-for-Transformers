"""
trick: loop-unet-full — Looped encoder (shared params, repeated) + unique decoder with skip_weights
...
class Hyperparameters:
    ...
    # trick: loop-unet-full — looped encoder + unique decoder hyperparameters
...
class GPT(nn.Module):
    def __init__(
        ...
        # trick: loop-unet-full — looped encoder + unique decoder with skip_weights
        ...
        self.num_skip_weights = min(num_loop_repeats, num_decoder)
...
def main() -> None:
    ...
    # trick: loop-unet-full — collect params from both encoder and decoder blocks
    ...
    log0(f"trick:loop-unet-full loop_layers:{args.num_loop_layers} loop_repeats:{args.num_loop_repeats} decoder_layers:{base_model.num_decoder_layers} skip_weights:{base_model.num_skip_weights}")
