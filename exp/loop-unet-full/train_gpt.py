...
trick: loop-unet-full — Loop the entire encoder half of the U-Net.
All encoder layers share parameters and are repeated (4 layers x 3 repeats),
while the decoder stays unique (5 layers). Skip connections are stored after
each loop iteration, giving 3 skip_weights connecting to the first 3 decoder
layers. Effective depth: 4*3 + 5 = 17, unique layers: 4 + 5 = 9.
"""
...
class Hyperparameters:
    ...
    # trick: loop-unet-full
    num_loop_layers = int(os.environ.get("NUM_LOOP_LAYERS", 4))  # trick: loop-unet-full — encoder layers in the looped block
    num_loop_repeats = int(os.environ.get("NUM_LOOP_REPEATS", 3))  # trick: loop-unet-full — how many times to repeat
...
class GPT(nn.Module):
    def __init__(
        ...
        rope_base: float,
        qk_gain_init: float,
        num_loop_layers: int = 4,  # trick: loop-unet-full
        num_loop_repeats: int = 3,  # trick: loop-unet-full
    ):
        super().__init__()
        ...
        # trick: loop-unet-full — looped encoder + unique decoder
        self.num_loop_layers = num_loop_layers  # trick: loop-unet-full
        self.num_loop_repeats = num_loop_repeats  # trick: loop-unet-full

        # Looped encoder block (shared parameters, repeated)
        self.encoder_blocks = nn.ModuleList([  # trick: loop-unet-full
            Block(model_dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init)
            for _ in range(num_loop_layers)
        ])

        # Decoder layers (unique)
        num_decoder = num_layers - num_loop_layers  # trick: loop-unet-full — 9 - 4 = 5
        self.num_decoder_layers = num_decoder  # trick: loop-unet-full
        # One skip per loop iteration, up to num_decoder
        self.num_skip_weights = min(num_loop_repeats, num_decoder)  # trick: loop-unet-full
        self.skip_weights = nn.Parameter(torch.ones(self.num_skip_weights, model_dim, dtype=torch.float32))  # trick: loop-unet-full
        self.decoder_blocks = nn.ModuleList([  # trick: loop-unet-full
            Block(model_dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        # trick: loop-unet-full — Looped encoder: store skip after each full iteration
        for rep in range(self.num_loop_repeats):
            for block in self.encoder_blocks:
                x = block(x, x0)
            skips.append(x)

        # trick: loop-unet-full — Decoder with skip connections (reverse order)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        num_loop_layers=args.num_loop_layers,  # trick: loop-unet-full
        num_loop_repeats=args.num_loop_repeats,  # trick: loop-unet-full
    ).to(device).bfloat16()
    ...
    # trick: loop-unet-full — collect params from all block groups
    all_block_params = (
        list(base_model.encoder_blocks.named_parameters()) +  # trick: loop-unet-full
        list(base_model.decoder_blocks.named_parameters())   # trick: loop-unet-full
    )
    block_named_params = all_block_params  # trick: loop-unet-full
    matrix_params = [
        p
    ...
    log0(f"trick:loop-unet-full loop_layers:{args.num_loop_layers} loop_repeats:{args.num_loop_repeats} effective_depth:{args.num_loop_layers * args.num_loop_repeats + (args.num_layers - args.num_loop_layers)} skip_weights:{base_model.num_skip_weights}")  # trick: loop-unet-full
