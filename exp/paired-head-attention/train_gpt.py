"""
GPT with Paired Head Attention trick.

trick: paired-head-attention — interleave pairs of adjacent heads in the sequence
dimension, creating 2×T sequences. Each query can attend to keys/values from both
heads in the pair, enriching the attention pattern. Applied to user-specified layers.

Official implementation from modded-nanogpt 2026-01-07_PairedHeadAttention.
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # trick: paired-head-attention — interleave adjacent head pairs in seq dim
        if self.use_paired_head:
            q = q.reshape(bsz, self.num_heads // 2, 2, seqlen, self.head_dim).transpose(2, 3).reshape(bsz, self.num_heads // 2, 2 * seqlen, self.head_dim)
            k = k.reshape(bsz, self.num_kv_heads // 2, 2, seqlen, self.head_dim).transpose(2, 3).reshape(bsz, self.num_kv_heads // 2, 2 * seqlen, self.head_dim)
            v = v.reshape(bsz, self.num_kv_heads // 2, 2, seqlen, self.head_dim).transpose(2, 3).reshape(bsz, self.num_kv_heads // 2, 2 * seqlen, self.head_dim)
        ...
        if self.use_paired_head:
            y = y.reshape(bsz, self.num_heads // 2, seqlen, 2, self.head_dim).transpose(2, 3).reshape(bsz, self.num_heads, seqlen, self.head_dim)
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, use_paired_head)
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    mlp_mult,
                    rope_base,
                    use_paired_head=(i in paired_set),
                )
