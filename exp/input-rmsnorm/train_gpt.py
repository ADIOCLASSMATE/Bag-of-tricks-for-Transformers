"""
Baseline GPT + input RMSNorm ablation.

Same as baseline (naive architecture + strong training recipe), but adds
RMSNorm on the embedding output before feeding into transformer blocks.

trick: input-rmsnorm — applies F.rms_norm to the token embedding output
before the first transformer block: x = F.rms_norm(tok_emb(input_ids), ...).
This normalizes the raw embedding activations.
"""
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        x = F.rms_norm(x, (x.size(-1),))  # trick: input-rmsnorm
