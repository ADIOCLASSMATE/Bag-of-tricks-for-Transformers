...
class Block(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.layer_idx = layer_idx  # trick: drop-attn — use a static first-layer branch
...
class GPT(nn.Module):
    def __init__(
        ...
        self._init_weights()
        self._freeze_static_drop_attn_params()
    ...
    def _freeze_static_drop_attn_params(self) -> None:
        # Block 0 never executes its attention branch in this experiment, so those
        # parameters must be frozen to keep DDP/optimizer state aligned with forward().
        if len(self.blocks) == 0:
            return
        first_block = self.blocks[0]
        for param in first_block.attn_norm.parameters():
            param.requires_grad_(False)
        for param in first_block.attn.parameters():
            param.requires_grad_(False)
...
def main() -> None:
    ...
    block_named_params = [
        ...
        if p.requires_grad and not name.startswith("0.attn_norm.") and not name.startswith("0.attn.")
    ]
    matrix_params = [
        p
