"""
GPT with drop-attn trick: disable attention in the first transformer block.

The first block's attention sublayer is replaced with a zero output, so the
residual connection effectively passes the input through unchanged. The first
block's attention parameters are frozen and excluded from the optimizer to
save compute. All other architecture and training recipe matches the baseline.
"""
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        # trick: drop-attn — remove attention only in the first transformer block
        attn_out = x.new_zeros(x.shape) if self.layer_idx == 0 else self.attn(self.attn_norm(x))
        x = x + attn_out
...
class GPT(nn.Module):
    ...
    def _freeze_static_drop_attn_params(self) -> None:
        """Freeze first block's attention params since they are never used in forward."""
        for p in self.blocks[0].attn.parameters():
...
def main() -> None:
    ...
    # trick: drop-attn — freeze first block's attention params
    base_model._freeze_static_drop_attn_params()
    ...
    # trick: drop-attn — exclude first block's attn params from optimizer
    block0_attn_names = {f"0.attn.{n}" for n, _ in base_model.blocks[0].attn.named_parameters()}
    block_named_params = [
        (name, p) for name, p in base_model.blocks.named_parameters()
        if name not in block0_attn_names
    ]
    matrix_params = [
        p
