...
class Hyperparameters:
    ...
    # trick: loop-stable-A — looped transformer hyperparameters
    num_loop_layers = int(os.environ.get("NUM_LOOP_LAYERS", 3))  # trick: loop-stable-A
    num_loop_repeats = int(os.environ.get("NUM_LOOP_REPEATS", 3))  # trick: loop-stable-A
    loop_stable_a = bool(int(os.environ.get("LOOP_STABLE_A", "1")))  # trick: loop-stable-A — enable stable A parameterization
    loop_log_a_init = float(os.environ.get("LOOP_LOG_A_INIT", "0.0"))  # trick: loop-stable-A — init for log_A parameter
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,log_A",
    ).split(",")
    if pattern
)
...
class GPT(nn.Module):
    def __init__(
        ...
        rope_base: float,
        qk_gain_init: float,
        num_loop_layers: int,  # trick: loop-stable-A
        num_loop_repeats: int,  # trick: loop-stable-A
        loop_stable_a: bool,  # trick: loop-stable-A
        loop_log_a_init: float,  # trick: loop-stable-A
    ):
        super().__init__()
        ...
        # trick: loop-stable-A — looped transformer: shared blocks repeated, no U-Net, no skip connections
        self.num_loop_layers = num_loop_layers  # trick: loop-stable-A
        self.num_loop_repeats = num_loop_repeats  # trick: loop-stable-A

        self.loop_blocks = nn.ModuleList([  # trick: loop-stable-A
            Block(
                model_dim,
                num_heads,
                num_kv_heads,
        ...
        # A_bar = diag(exp(-exp(log_A))) guarantees rho(A_bar) < 1 (spectral radius < 1)
        # B_bar = I - A_bar = diag(1 - exp(-exp(log_A)))
        # With log_A_init=0.0: A_bar ~ 0.37, B_bar ~ 0.63
        self.loop_stable_a = loop_stable_a  # trick: loop-stable-A
        if loop_stable_a:
            self.log_A = nn.Parameter(torch.full((model_dim,), loop_log_a_init, dtype=torch.float32))  # trick: loop-stable-A
        else:
            self.log_A = None  # trick: loop-stable-A — disabled, falls back to loop-pure behavior
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        x0 = x
        h = x  # trick: loop-stable-A — initial hidden state

        # trick: loop-stable-A — compute stable transition matrices from learnable log_A
        if self.log_A is not None:
            # A_bar = diag(exp(-exp(log_A))), each diagonal entry in (0, 1) — guaranteed stable
            a_bar = torch.exp(-torch.exp(self.log_A.to(dtype=h.dtype)))[None, None, :]  # (1, 1, dim)  # trick: loop-stable-A
            b_bar = 1.0 - a_bar  # (1, 1, dim) — B_bar = I - A_bar  # trick: loop-stable-A

        # trick: loop-stable-A — looped forward with stable state transition between iterations
        for rep in range(self.num_loop_repeats):
            for block in self.loop_blocks:
                h = block(h, x0)
            # trick: loop-stable-A — apply stable recurrence between iterations
            # h_{t+1} = A_bar * h_t + B_bar * x0, where x0 is the original embedding
            if self.log_A is not None:
                h = a_bar * h + b_bar * x0  # trick: loop-stable-A

        x = self.final_norm(h).reshape(-1, h.size(-1))
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        num_layers=args.num_layers,  # kept for compatibility but not used for block count  # trick: loop-stable-A
        ...
        num_loop_layers=args.num_loop_layers,  # trick: loop-stable-A
        num_loop_repeats=args.num_loop_repeats,  # trick: loop-stable-A
        loop_stable_a=args.loop_stable_a,  # trick: loop-stable-A
        loop_log_a_init=args.loop_log_a_init,  # trick: loop-stable-A
    ).to(device).bfloat16()
    ...
    block_named_params = list(base_model.loop_blocks.named_parameters())  # trick: loop-stable-A — collect params from loop blocks only
    matrix_params = [
        p
    ...
    # trick: loop-stable-A — log_A is a scalar/control parameter, add to scalar_params
    if hasattr(base_model, 'log_A') and base_model.log_A is not None:
        scalar_params.append(base_model.log_A)  # trick: loop-stable-A
    # trick: loop-stable-A — skip_weights is empty, no need to add
    ...
    log0(f"trick:loop-stable-A loop_layers:{args.num_loop_layers} loop_repeats:{args.num_loop_repeats} stable_a:{args.loop_stable_a} log_a_init:{args.loop_log_a_init} effective_depth:{args.num_loop_layers * args.num_loop_repeats}")  # trick: loop-stable-A
    # trick: loop-stable-A — log initial A_bar values
    if base_model.log_A is not None:
        with torch.no_grad():
            init_a_bar = torch.exp(-torch.exp(base_model.log_A)).mean().item()
        log0(f"trick:loop-stable-A init_A_bar_mean:{init_a_bar:.4f} init_B_bar_mean:{1.0 - init_a_bar:.4f}")  # trick: loop-stable-A
    ...
    while True:
        ...
        if should_validate:
            ...
            # trick: loop-stable-A — log A_bar statistics at validation time
            if base_model.log_A is not None:
                with torch.no_grad():
                    a_bar_mean = torch.exp(-torch.exp(base_model.log_A)).mean().item()
                    a_bar_min = torch.exp(-torch.exp(base_model.log_A)).min().item()
                    a_bar_max = torch.exp(-torch.exp(base_model.log_A)).max().item()
                log0(f"trick:loop-stable-A A_bar mean:{a_bar_mean:.4f} min:{a_bar_min:.4f} max:{a_bar_max:.4f}")  # trick: loop-stable-A
                wandb_log({  # trick: loop-stable-A
                    "loop_stable_a/a_bar_mean": a_bar_mean,
                    "loop_stable_a/a_bar_min": a_bar_min,
                    "loop_stable_a/a_bar_max": a_bar_max,
                }, step=step)
            torch.cuda.synchronize()
            t0 = time.perf_counter()
