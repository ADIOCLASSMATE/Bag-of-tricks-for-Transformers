...
class Hyperparameters:
    ...
    # trick: loop-pure — looped transformer hyperparameters
    num_loop_layers = int(os.environ.get("NUM_LOOP_LAYERS", 3))  # trick: loop-pure
    num_loop_repeats = int(os.environ.get("NUM_LOOP_REPEATS", 3))  # trick: loop-pure
...
class GPT(nn.Module):
    def __init__(
        ...
        rope_base: float,
        qk_gain_init: float,
        num_loop_layers: int,  # trick: loop-pure
        num_loop_repeats: int,  # trick: loop-pure
    ):
        super().__init__()
        ...
        # trick: loop-pure — pure looped transformer: shared block repeated, no U-Net, no skip connections
        self.num_loop_layers = num_loop_layers  # trick: loop-pure
        self.num_loop_repeats = num_loop_repeats  # trick: loop-pure

        self.loop_blocks = nn.ModuleList([  # trick: loop-pure
            Block(
                model_dim,
                num_heads,
                num_kv_heads,
        ...
        # No skip_weights — pure loop has no U-Net skip connections  # trick: loop-pure
        self.num_skip_weights = 0  # trick: loop-pure
        self.skip_weights = nn.Parameter(torch.empty(0, model_dim, dtype=torch.float32))  # trick: loop-pure — dummy, no skips
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        # trick: loop-pure — pure looped forward pass, no U-Net skip connections
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        num_layers=args.num_layers,  # kept for compatibility but not used for block count  # trick: loop-pure
        ...
        num_loop_layers=args.num_loop_layers,  # trick: loop-pure
        num_loop_repeats=args.num_loop_repeats,  # trick: loop-pure
    ).to(device).bfloat16()
    ...
    block_named_params = list(base_model.loop_blocks.named_parameters())  # trick: loop-pure — collect params from loop blocks only
    matrix_params = [
        p
    ...
    # trick: loop-pure — skip_weights is empty, no need to add
    ...
    log0(f"trick:loop-pure loop_layers:{args.num_loop_layers} loop_repeats:{args.num_loop_repeats} effective_depth:{args.num_loop_layers * args.num_loop_repeats}")  # trick: loop-pure
