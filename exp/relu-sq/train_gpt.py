"""
Baseline GPT + ReLU^2 MLP ablation.

Same as baseline (naive architecture + strong training recipe), but replaces
GELU activation with ReLU^2 (relu then square) in the MLP.

trick: ReLU^2 MLP — uses x = torch.relu(fc(x)); proj(x.square()) instead of
proj(F.gelu(fc(x))). The squared ReLU activation has been shown to be a
cheaper alternative to GELU with competitive performance.
"""
...
class MLP(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:  # trick: relu-sq
        x = torch.relu(self.fc(x))  # trick: relu-sq
        return self.proj(x.square())  # trick: relu-sq
