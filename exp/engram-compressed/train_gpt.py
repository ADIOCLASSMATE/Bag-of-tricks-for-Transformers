"""
Naive architecture GPT with strong training recipe + engram memory branches with tokenizer compression.
...
Engram trick (with tokenizer compression): n-gram memory branches inserted at specific transformer layers. Each EngramBranch
hashes input token n-grams into a multi-head embedding table, gates the retrieved values against the hidden state, and adds
the result as a residual. Includes short-convolution mixing. Tokenizer compression maps the original token vocabulary to a
smaller set by normalizing and deduplicating token strings, reducing the hash vocabulary size for engram lookups.
...
class Hyperparameters:
    ...
    # trick: engram-compressed — tokenizer compression enabled by default
    engram_use_tokenizer_compression = bool(
        int(os.environ.get("ENGRAM_USE_TOKENIZER_COMPRESSION", "1"))
    )
...
class EngramBranch(nn.Module):
    # trick: engram memory branch with tokenizer compression
    def __init__(
        self,
        dim: int,
        layer_id: int,
...
def main() -> None:
    ...
    # trick: engram-compressed — Conv1d weight is 3D, falls through ndim==2 and ndim<2 filters
