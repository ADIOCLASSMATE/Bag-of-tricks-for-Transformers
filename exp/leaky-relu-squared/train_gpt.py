...
class MLP(nn.Module):
    # trick: leaky_relu(x, 0.5).square() replaces relu(x).square()
    # Preserves negative gradient flow, eliminates dead neurons, while
    # squaring still produces non-negative outputs like relu^2.
    ...
    def forward(self, x: Tensor) -> Tensor:
        # trick: leaky_relu(x, negative_slope=0.5) instead of relu(x)
