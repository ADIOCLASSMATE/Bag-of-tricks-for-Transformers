...
Training recipe (identical to baseline):
  - Muon optimizer for 2D matrix params in transformer blocks (encoder + decoder)
...
class Hyperparameters(_BaseHyperparameters):
    # trick: loop-share-first — fraction of first blocks shared (looped)
    num_loop_layers = float(os.environ.get("NUM_LOOP_LAYERS", "0.4444"))
    num_loop_repeats = int(os.environ.get("NUM_LOOP_REPEATS", "3"))


# -----------------------------
# TRANSFORMER MODULES (custom GPT for loop-share-first)
# -----------------------------
