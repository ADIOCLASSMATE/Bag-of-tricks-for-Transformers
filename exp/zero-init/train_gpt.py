"""
Baseline GPT + zero-init output projections ablation.

Same as baseline (naive architecture + strong training recipe), but
zero-initializes the output projection layers (attn.proj, mlp.proj).

trick: zero-init — sets _zero_init flag on output projections so they start
as identity mappings (x + 0 = x), making each block initially a no-op.
This stabilizes early training by preserving the signal from lower layers.
"""
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.proj._zero_init = True  # trick: zero-init
...
class GPT(nn.Module):
    def __init__(
        ...
        self._init_weights()  # trick: zero-init

    def _init_weights(self) -> None:  # trick: zero-init
