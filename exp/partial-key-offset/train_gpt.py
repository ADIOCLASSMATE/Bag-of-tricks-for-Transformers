"""
GPT with Partial Key Offset trick.

trick: partial-key-offset — shift stationary key dimensions forward by 1 position,
enabling single-layer induction head formation.

trick: partial-key-offset dependency — Half-Truncated RoPE: only rotate first
head_dim//4 dims, leaving the rest stationary. Required so that key offset can
target only the stationary dims.

Official implementation from modded-nanogpt 2025-12-14_PartialKeyOffset.

Baseline architecture: GQA, RoPE, RMSNorm, QK-Norm, Tied Embeddings,
CastedLinear, GELU MLP, Muon+Adam optimizer.
"""
...
class Rotary(nn.Module):
    def __init__(self, dim: int, base: float = 10000.0, half_truncate: bool = False):
        super().__init__()
        # trick: partial-key-offset dependency — half-truncated RoPE
        # Only rotate first dim//4 dims; rest stay stationary so key offset targets them
        if half_truncate:
            steps = dim // 4
            inv_freq = 1.0 / (base ** (torch.arange(0, steps * 2, 2, dtype=torch.float32) / (steps * 2)))
            inv_freq = torch.cat([inv_freq, inv_freq.new_zeros(dim // 2 - steps)])
        else:
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        num_kv_heads: int,
        rope_base: float,
        use_key_offset: bool = False,
        half_truncate: bool = False,
    ):
        super().__init__()
        if dim % num_heads != 0:
        ...
        self.rotary = Rotary(self.head_dim, base=rope_base, half_truncate=half_truncate)
        # trick: partial-key-offset — enable key shift on this layer
        self.use_key_offset = use_key_offset
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # trick: partial-key-offset — shift stationary key dims forward by 1 position
        if self.use_key_offset and seqlen > 1:
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, use_key_offset, half_truncate)
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    mlp_mult,
                    rope_base,
                    use_key_offset=(i in offset_set),
                    half_truncate=half_truncate,
                )
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        key_offset_layers=args.key_offset_layers,
        half_truncate=args.rope_half_truncate,
    ).to(device).bfloat16()
