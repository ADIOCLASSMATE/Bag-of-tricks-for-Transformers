"""
Naive architecture GPT with strong training recipe + KV Sharing + Double-Wide MLP.

Trick: Last N layers share KV projections with earlier layers + double-wide MLP.
Shared KV layers reuse the c_k and c_v projections from their source layers,
and use an MLP with doubled hidden dimension to use the saved parameter budget.
