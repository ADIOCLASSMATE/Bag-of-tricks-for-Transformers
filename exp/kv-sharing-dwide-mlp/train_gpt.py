...
trick: kv-sharing-dwide-mlp — Last N layers share KV projections with earlier layers,
and get a wider MLP to use the saved parameter budget.
Inspired by Gemma4's num_kv_shared_layers + double-wide MLP design.
"""
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        rope_base: float,
        qk_gain_init: float,
        kv_shared: bool = False,  # trick: kv-sharing-dwide-mlp
    ):
        super().__init__()
        if dim % num_heads != 0:
        ...
        self.kv_shared = kv_shared  # trick: kv-sharing-dwide-mlp
        self.c_q = CastedLinear(dim, dim, bias=False)
        if not kv_shared:  # trick: kv-sharing-dwide-mlp
            self.c_k = CastedLinear(dim, kv_dim, bias=False)
            self.c_v = CastedLinear(dim, kv_dim, bias=False)
        else:  # trick: kv-sharing-dwide-mlp
            self.c_k = None  # trick: kv-sharing-dwide-mlp — shared layers don't own KV
            self.c_v = None
    ...
    def forward(  # trick: kv-sharing-dwide-mlp
        self, x: Tensor, shared_k: Tensor | None = None, shared_v: Tensor | None = None  # trick: kv-sharing-dwide-mlp
    ) -> tuple[Tensor, Tensor | None, Tensor | None]:  # trick: kv-sharing-dwide-mlp
        ...
        if not self.kv_shared:  # trick: kv-sharing-dwide-mlp
            k = self.c_k(x).reshape(bsz, seqlen, self.num_kv_heads, self.head_dim).transpose(1, 2)
            k = F.rms_norm(k, (k.size(-1),))
            k = apply_rotary_emb(k, cos, sin)
            v = self.c_v(x).reshape(bsz, seqlen, self.num_kv_heads, self.head_dim).transpose(1, 2)
        else:
            k = shared_k  # trick: kv-sharing-dwide-mlp — use shared KV
            v = shared_v
        ...
        k_share = k if not self.kv_shared else None  # trick: kv-sharing-dwide-mlp
        v_share = v if not self.kv_shared else None
        return self.proj(y), k_share, v_share


class MLP(nn.Module):
    # relu^2 MLP from the original modded-nanogpt setup
    def __init__(self, dim: int, mlp_mult: int, mlp_mult_override: int | None = None):  # trick: kv-sharing-dwide-mlp
        super().__init__()
        hidden = (mlp_mult_override or mlp_mult) * dim  # trick: kv-sharing-dwide-mlp
...
class Block(nn.Module):
    def __init__(
        ...
        qk_gain_init: float,
        kv_shared: bool = False,  # trick: kv-sharing-dwide-mlp
        mlp_mult_override: int | None = None,  # trick: kv-sharing-dwide-mlp
    ):
        super().__init__()
        self.attn_norm = RMSNorm()
        self.mlp_norm = RMSNorm()
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init, kv_shared=kv_shared)  # trick: kv-sharing-dwide-mlp
        self.mlp = MLP(dim, mlp_mult, mlp_mult_override=mlp_mult_override)  # trick: kv-sharing-dwide-mlp
        self.attn_scale = nn.Parameter(torch.ones(dim, dtype=torch.float32))
        self.mlp_scale = nn.Parameter(torch.ones(dim, dtype=torch.float32))
        self.resid_mix = nn.Parameter(torch.stack((torch.ones(dim), torch.zeros(dim))).float())

    def forward(  # trick: kv-sharing-dwide-mlp
        self, x: Tensor, x0: Tensor, shared_k: Tensor | None = None, shared_v: Tensor | None = None  # trick: kv-sharing-dwide-mlp
    ) -> tuple[Tensor, Tensor | None, Tensor | None]:  # trick: kv-sharing-dwide-mlp
        mix = self.resid_mix.to(dtype=x.dtype)
        x = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        attn_out, k_share, v_share = self.attn(self.attn_norm(x), shared_k, shared_v)  # trick: kv-sharing-dwide-mlp
        x = x + self.attn_scale.to(dtype=x.dtype)[None, None, :] * attn_out
        x = x + self.mlp_scale.to(dtype=x.dtype)[None, None, :] * self.mlp(self.mlp_norm(x))
        return x, k_share, v_share  # trick: kv-sharing-dwide-mlp
...
class GPT(nn.Module):
    def __init__(
        ...
        rope_base: float,
        qk_gain_init: float,
        num_kv_shared_layers: int = 2,  # trick: kv-sharing-dwide-mlp
        use_double_wide_mlp: bool = True,  # trick: kv-sharing-dwide-mlp
    ):
        super().__init__()
        ...
        # trick: kv-sharing-dwide-mlp
        first_kv_shared_layer = num_layers - num_kv_shared_layers if num_kv_shared_layers > 0 else num_layers  # trick: kv-sharing-dwide-mlp
        self.kv_source_map: dict[int, int] = {}  # trick: kv-sharing-dwide-mlp
        for i in range(first_kv_shared_layer, num_layers):  # trick: kv-sharing-dwide-mlp
            source_idx = i - first_kv_shared_layer  # trick: kv-sharing-dwide-mlp
            self.kv_source_map[i] = source_idx  # trick: kv-sharing-dwide-mlp
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    rope_base,
                    qk_gain_init,
                    kv_shared=(i >= first_kv_shared_layer and num_kv_shared_layers > 0),  # trick: kv-sharing-dwide-mlp
                    mlp_mult_override=(mlp_mult * 2 if use_double_wide_mlp and i >= first_kv_shared_layer and num_kv_shared_layers > 0 else None),  # trick: kv-sharing-dwide-mlp
                )
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        # trick: kv-sharing-dwide-mlp
        kv_cache: dict[int, tuple[Tensor, Tensor]] = {}  # trick: kv-sharing-dwide-mlp

        # First half stores skips; second half reuses them in reverse order.
        for i in range(self.num_encoder_layers):
            x, k_share, v_share = self.blocks[i](x, x0)  # trick: kv-sharing-dwide-mlp
            if k_share is not None:  # trick: kv-sharing-dwide-mlp
                kv_cache[i] = (k_share, v_share)  # trick: kv-sharing-dwide-mlp
            skips.append(x)
        for i in range(self.num_decoder_layers):
            if skips:
                x = x + self.skip_weights[i].to(dtype=x.dtype)[None, None, :] * skips.pop()
            block_idx = self.num_encoder_layers + i  # trick: kv-sharing-dwide-mlp
            shared_k, shared_v = None, None  # trick: kv-sharing-dwide-mlp
            if block_idx in self.kv_source_map:  # trick: kv-sharing-dwide-mlp
                source = self.kv_source_map[block_idx]  # trick: kv-sharing-dwide-mlp
                if source in kv_cache:  # trick: kv-sharing-dwide-mlp
                    shared_k, shared_v = kv_cache[source]  # trick: kv-sharing-dwide-mlp
            x, _, _ = self.blocks[block_idx](x, x0, shared_k, shared_v)  # trick: kv-sharing-dwide-mlp
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        num_kv_shared_layers=args.num_kv_shared_layers,  # trick: kv-sharing-dwide-mlp
        use_double_wide_mlp=args.use_double_wide_mlp,  # trick: kv-sharing-dwide-mlp
    ).to(device).bfloat16()
    ...
    log0(f"trick:kv-sharing-dwide-mlp num_kv_shared_layers:{args.num_kv_shared_layers} use_double_wide_mlp:{args.use_double_wide_mlp} source_map:{base_model.kv_source_map}")  # trick: kv-sharing-dwide-mlp
