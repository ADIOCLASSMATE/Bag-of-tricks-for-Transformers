"""
Trick: kv-sharing-dwide-mlp — Cross-layer KV sharing + double-wide MLP (combined).

Last N layers (i) reuse c_k and c_v projections from earlier layers (i - first_kv_shared_layer)
and (ii) use an MLP with doubled hidden dimension. This is the bundled trick from Gemma-style
designs; it is NOT parameter-neutral (saves c_k+c_v but adds ~3x more MLP params per shared layer,
net +9.2% params on small).

To attribute the contributions, see the companion ablations:
  - exp/kv-sharing-only       — KV sharing only, MLP width unchanged
  - exp/dwide-mlp-only        — Double-wide MLP only, no KV sharing
