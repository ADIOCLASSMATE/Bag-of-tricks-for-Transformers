"""
Trick: resid-mix-init11 — resid-mix with non-identity init [1.1, 0.1].

Architecture is identical to resid-mix. The only difference is the initialization
of the resid_mix tensor:
  resid-mix:        init [1.0, 0.0]  — identity-equivalent at step 0
  resid-mix-init11: init [1.1, 0.1]  — non-identity, biases each layer to mix in
                                       10% of the original embedding

The intended comparison for this experiment is vs `resid-mix` (same architecture,
different init), NOT vs baseline (which has no resid_mix at all).

Training recipe (identical to baseline / resid-mix):
...
# resid-mix-init11 uses non-empty CONTROL_TENSOR_NAME_PATTERNS default to keep
...
# -----------------------------
# TRANSFORMER MODULES (custom Block + GPT for resid-mix-init11)
# -----------------------------
...
class Block(nn.Module):
    def __init__(
        ...
        # trick: resid-mix-init11 — non-identity init: residual=1.1, x0=0.1
        self.resid_mix = nn.Parameter(torch.stack((torch.full((dim,), 1.1), torch.full((dim,), 0.1))).float())
