"""
GPT with Multi-Token Prediction (MTP) trick.

trick: multi-token-prediction — Uses the same logits to predict multiple future tokens
with a weighted cross-entropy loss. A 3-phase schedule anneals from 3-token prediction
to standard next-token prediction. Zero additional parameters.

Official implementation from modded-nanogpt 2025-12-22_MultiTokenPrediction.
...
class Hyperparameters:
    ...
    # trick: multi-token-prediction — enable weighted CE over next k tokens
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, mtp_weights: Tensor | None = None) -> Tensor:
        ...
        # trick: multi-token-prediction — weighted CE over next k tokens
        if mtp_weights is not None and mtp_weights.size(0) > 1:
            n_predict = mtp_weights.size(0)
            logits_f = logits.view(-1, logits.size(-1)).float()
            targets_flat = target_ids.reshape(-1)
            idx = F.pad(targets_flat, (0, n_predict - 1)).unfold(0, n_predict, 1)
            target_logits = logits_f.gather(1, idx)
            ce = torch.logsumexp(logits_f, dim=-1).unsqueeze(1) - target_logits
            for k in range(1, n_predict):
                ce[-k:, k] = 0
            return (ce * mtp_weights.to(logits_f.device)).sum()
...
def main() -> None:
    ...
    # trick: multi-token-prediction — precompute weight schedule
    mtp_schedule: list[Tensor] = []
    if args.mtp_enabled:
        for s in range(args.iterations + 1):
            ...
            if x < 1 / 3:
                ...
                w = [1.0]
            mtp_schedule.append(torch.tensor(w, device=device))
        log0(f"mtp_schedule: {len(mtp_schedule)} steps, phase_1: 3-token, phase_2: 2-token, phase_3: 1-token")
    ...
    while True:
        ...
        for micro_step in range(grad_accum_steps):
            ...
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y, mtp_schedule[step] if args.mtp_enabled else None)
