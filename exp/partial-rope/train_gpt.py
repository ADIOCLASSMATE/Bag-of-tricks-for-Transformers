"""
GPT with partial RoPE trick: apply rotary position embeddings to only a
configurable subset of head dimensions.

When ROPE_DIMS < head_dim, only the first rope_dims dimensions receive rotary
position embeddings while the remaining dimensions pass through unchanged.
This allows the model to retain some position-free representational capacity.
Set ROPE_DIMS=0 (default) to use full RoPE (all head_dim dimensions).
