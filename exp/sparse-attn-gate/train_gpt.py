"""
GPT with Sparse Attention Gate trick.

trick: sparse-attn-gate — low-dimensional per-head sigmoid gating on attention output.
A compact linear layer (default 12 input dims) maps a small slice of the residual stream
to per-head gates, allowing each head to be selectively dampened based on context.

Official implementation from modded-nanogpt 2025-08-23_SparseAttnGate.
...
class Hyperparameters:
    ...
    # trick: sparse-attn-gate — input dimension for the per-head gate
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        # trick: sparse-attn-gate — low-dim per-head gate on attention output
        gate_input_dim = int(os.environ.get("ATTN_GATE_DIM", 12))
        self.attn_gate = CastedLinear(gate_input_dim, num_heads, bias=False)
        nn.init.zeros_(self.attn_gate.weight)

    def forward(self, x: Tensor) -> Tensor:
        bsz, seqlen, dim = x.shape
        # trick: sparse-attn-gate — compute per-head gate from low-dim slice of input
        gate_input_dim = self.attn_gate.in_features
        gate = torch.sigmoid(
            self.attn_gate(x[..., :gate_input_dim])
        ).view(bsz, seqlen, self.num_heads, 1)
        ...
        # trick: sparse-attn-gate — apply per-head gating
        y = y * gate.transpose(1, 2).to(dtype=y.dtype)
