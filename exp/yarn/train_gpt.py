"""
GPT with YaRN (Yet another RoPE extensioN) trick.

trick: yarn — Frequency interpolation for RoPE that extends the effective context
window beyond the training sequence length. Uses a ramp function to smoothly
interpolate frequencies: high frequencies (short-range) are linearly scaled,
low frequencies (long-range) keep original values.

Official implementation from parameter-golf A14 (static YaRN).
...
class Hyperparameters:
    ...
    # trick: yarn — YaRN frequency interpolation for long-context extrapolation
    use_yarn = bool(int(os.environ.get("USE_YARN", "0")))
...
class Rotary(nn.Module):
    def __init__(self, dim: int, base: float = 10000.0, use_yarn: bool = False, yarn_max_len: int = 4096, train_seq_len: int = 1024):
        super().__init__()
        inv_freq = 1.0 / (base ** (torch.arange(0, dim, 2, dtype=torch.float32) / dim))
        # trick: yarn — YaRN frequency interpolation for long-context extrapolation
        if use_yarn and train_seq_len < yarn_max_len:
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.rotary = Rotary(self.head_dim, base=rope_base, use_yarn=use_yarn, yarn_max_len=yarn_max_len, train_seq_len=train_seq_len)
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, use_yarn, yarn_max_len, train_seq_len)
