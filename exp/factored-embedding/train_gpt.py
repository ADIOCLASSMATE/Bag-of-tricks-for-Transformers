"""
GPT with Factored Embedding trick.

trick: factored-embedding — Use a smaller embedding dimension (embed_dim) with a
linear projection to model_dim, instead of a full V × model_dim embedding table.
Requires untied lm_head since embed_dim != model_dim.

Total params: V × embed_dim + embed_dim × model_dim + model_dim × V
vs baseline: V × model_dim (tied) or 2 × V × model_dim (untied).

Official implementation from parameter-golf A12.

Baseline architecture: GQA, RoPE, RMSNorm, QK-Norm, GELU MLP, Muon+Adam optimizer.
"""
...
class Hyperparameters:
    ...
    # trick: factored-embedding — embedding dimension (must be <= model_dim)
...
class GPT(nn.Module):
    def __init__(
        ...
        super().__init__()
        # trick: factored-embedding — smaller embed dim + linear proj to model_dim
        self.embed_dim = embed_dim
        self.tok_emb = nn.Embedding(vocab_size, embed_dim)
        self.embed_proj = CastedLinear(embed_dim, model_dim, bias=False)
        # Factored embedding requires untied lm_head (embed_dim != model_dim breaks tie)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        x = self.embed_proj(x)  # trick: factored-embedding — project embed_dim → model_dim
...
def main() -> None:
    ...
    # trick: factored-embedding — embed_proj is a matrix param, use Muon
    if hasattr(base_model, "embed_proj") and base_model.embed_proj is not None:
    ...
    token_lr = args.embed_lr  # trick: factored-embedding requires untied embeddings
