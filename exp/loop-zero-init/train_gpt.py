...
class Hyperparameters:
    ...
    # trick: loop-zero-init -- looped transformer hyperparameters
    num_loop_layers = int(os.environ.get("NUM_LOOP_LAYERS", 3))  # trick: loop-zero-init
    num_loop_repeats = int(os.environ.get("NUM_LOOP_REPEATS", 3))  # trick: loop-zero-init
    loop_zero_init = bool(int(os.environ.get("LOOP_ZERO_INIT", "1")))  # trick: loop-zero-init -- zero-init output projections in loop blocks
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        rope_base: float,
        qk_gain_init: float,
        zero_init: bool = True,  # trick: loop-zero-init
    ):
        super().__init__()
        if dim % num_heads != 0:
        ...
        self.proj._zero_init = zero_init  # trick: loop-zero-init
...
class MLP(nn.Module):
    ...
    def __init__(self, dim: int, mlp_mult: int, zero_init: bool = True):  # trick: loop-zero-init
        super().__init__()
        hidden = mlp_mult * dim
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init, zero_init=zero_init)  # trick: loop-zero-init
        self.mlp = MLP(dim, mlp_mult, zero_init=zero_init)  # trick: loop-zero-init
...
class GPT(nn.Module):
    def __init__(
        ...
        rope_base: float,
        qk_gain_init: float,
        num_loop_layers: int,  # trick: loop-zero-init
        num_loop_repeats: int,  # trick: loop-zero-init
        loop_zero_init: bool,  # trick: loop-zero-init
    ):
        super().__init__()
        ...
        # trick: loop-zero-init -- pure looped transformer: shared block repeated, no U-Net, no skip connections
        self.num_loop_layers = num_loop_layers  # trick: loop-zero-init
        self.num_loop_repeats = num_loop_repeats  # trick: loop-zero-init
        self.loop_zero_init = loop_zero_init  # trick: loop-zero-init

        self.loop_blocks = nn.ModuleList([  # trick: loop-zero-init
            Block(
                model_dim,
                num_heads,
                num_kv_heads,
                ...
                rope_base,
                qk_gain_init,
                zero_init=loop_zero_init,  # trick: loop-zero-init -- control zero-init for loop block output projections
            )
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        # trick: loop-zero-init -- pure looped forward pass, no U-Net skip connections
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        num_layers=args.num_layers,  # kept for compatibility but not used for block count  # trick: loop-zero-init
        ...
        num_loop_layers=args.num_loop_layers,  # trick: loop-zero-init
        num_loop_repeats=args.num_loop_repeats,  # trick: loop-zero-init
        loop_zero_init=args.loop_zero_init,  # trick: loop-zero-init
    ).to(device).bfloat16()
    ...
    block_named_params = list(base_model.loop_blocks.named_parameters())  # trick: loop-zero-init -- collect params from loop blocks only
    matrix_params = [
        p
    ...
    # trick: loop-zero-init -- skip_weights is empty, no need to add
    ...
    log0(f"trick:loop-zero-init loop_layers:{args.num_loop_layers} loop_repeats:{args.num_loop_repeats} loop_zero_init:{args.loop_zero_init} effective_depth:{args.num_loop_layers * args.num_loop_repeats}")  # trick: loop-zero-init
