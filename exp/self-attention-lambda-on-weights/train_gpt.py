"""
Self-Attention Lambda on V Weights -- GPT Training Script

This script simplifies the `track_1_short / 2025-12-10_SALambdaOnWeight` approach: instead of
multiplying the lambda on the merged QKV weight matrix, we apply it only to the V projection
weight (since Q and K are immediately RMS-normed).  The per-layer scalar sa_lambda (init 0.2,
multiplied by 5 at forward time) preserves the ~5x effective learning rate while avoiding the
overhead of scaling the full QKV matrix.

It is applicable for the merge of `V` and `token embeddings` as well.

Core modification -- SA-Lambda on V weights:
  - Per-layer learnable scalar sa_lambda (init 0.2, param stored in fp32) scales the V projection
    weight W_v before the linear transform: V = x @ (5 * sa_lambda * W_v).T
  - The factor-5 multiplier provides ~5x effective learning rate (gradient scaling) while keeping
    the effective initial value at 1.0, so training starts identically to the baseline.
  - Only V is modulated; Q and K are unchanged since QK-Norm would absorb any scalar scaling.
  - sa_lambda goes to the Adam optimizer (scalar group) along with other low-dimensional params.
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        # init 0.2, forward uses 5x → effective init = 1.0, grad scaled 5x
        self.sa_lambda = nn.Parameter(torch.tensor(0.2))
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # self-attention: λ on V weight with 5x gradient multiplier
        c_v_scaled = 5 * self.sa_lambda * self.c_v.weight
        v = F.linear(x, c_v_scaled.type_as(x)).reshape(bsz, seqlen, self.num_kv_heads, self.head_dim).transpose(1, 2)
