"""
Naive architecture GPT with strong training recipe + ortho-init trick.
...
Trick: ortho-init — orthogonalize every non-zero-init linear weight matrix via nn.init.orthogonal_.
...
class GPT(nn.Module):
    def __init__(
        ...
        self._init_weights()
        # trick: orthogonal init for Q/K/V and MLP fc (non-zero, non-lm_head layers)
        ...
        for name, module in self.named_modules():
            if isinstance(module, nn.Linear):
                if name.endswith('.proj'):
                    ...
                    # Q/K/V and MLP fc: PyTorch default, skip (ortho applied below)
                    pass
                else:
