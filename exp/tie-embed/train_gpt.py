"""
Naive architecture GPT with strong training recipe + weight-tied embeddings baseline.

Architecture: GQA, RoPE, RMSNorm (no learnable weight), QK-Norm, Tied Embeddings,
CastedLinear, restore_low_dim_params_to_fp32.

Baseline: tie-embed — weight-tied embedding and output lm_head matrices (standard
parameter-efficiency approach). This configuration uses tie_embeddings=1 and
tied_embed_lr=0.05, serving as the control baseline for embedding ablations.
...
Training recipe borrows from modded-nanogpt:
  ...
  - AdamW weight_decay=0.1 on embeddings, no weight decay on Muon/scalar params
...
class Hyperparameters:
    ...
    tie_embeddings = bool(int(os.environ.get("TIE_EMBEDDINGS", "1")))  # tie embedding & lm_head (baseline)
