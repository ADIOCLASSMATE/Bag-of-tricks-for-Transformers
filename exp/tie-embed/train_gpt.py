"""
Trick: tie-embed — weight-tied embedding and output lm_head matrices.

The only architectural difference vs baseline is `tie_embeddings=1`: the token
embedding matrix is reused as the output projection (no separate lm_head). All
other architectural and training choices match baseline exactly, including:
  - weight_decay = 0.0 on the (tied) token embedding (matches baseline's tok_emb)
  - separate `tied_embed_lr` (0.05) for the tied matrix instead of `embed_lr`

Training recipe (identical to baseline):
  - Muon optimizer for 2D matrix params in transformer blocks
  - AdamW for scalar/control params and the (tied) token embedding
  - Muon momentum warmup
  - Weight decay: 0.0 on token embedding, Muon, and scalar params (no separate lm_head exists)
