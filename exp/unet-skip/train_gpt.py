"""
Baseline GPT + U-Net skip connections ablation.

Same as baseline (naive architecture + strong training recipe), but adds U-Net
skip connections with learnable skip_weights. First half of layers store skip
activations; second half combines them in reverse order.

trick: U-Net skip connections — encoder layers store activations, decoder layers
receive them with learnable per-skip scaling (skip_weights, initialized to 1.0).
"""
...
# trick: unet-skip — skip_weights are control params (scalar, kept fp32, go to Adam not Muon)
...
class GPT(nn.Module):
    def __init__(
        ...
        self.num_encoder_layers = num_layers // 2  # trick: unet-skip
        self.num_decoder_layers = num_layers - self.num_encoder_layers  # trick: unet-skip
        self.num_skip_weights = min(self.num_encoder_layers, self.num_decoder_layers)  # trick: unet-skip
        self.skip_weights = nn.Parameter(torch.ones(self.num_skip_weights, model_dim, dtype=torch.float32))  # trick: unet-skip
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        skips: list[Tensor] = []  # trick: unet-skip

        # trick: unet-skip — First half stores skips; second half reuses them in reverse order.
        for i in range(self.num_encoder_layers):
            x = self.blocks[i](x)
            skips.append(x)
        for i in range(self.num_decoder_layers):
            if skips:
                x = x + self.skip_weights[i].to(dtype=x.dtype)[None, None, :] * skips.pop()  # trick: unet-skip
...
def main() -> None:
    ...
    # trick: unet-skip — skip_weights are scalar control params, go to Adam
    ...
    log0(f"trick:unet-skip encoder_layers:{base_model.num_encoder_layers} decoder_layers:{base_model.num_decoder_layers} skip_weights:{base_model.num_skip_weights}")
