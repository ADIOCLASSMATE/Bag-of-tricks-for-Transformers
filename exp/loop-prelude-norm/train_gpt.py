...
class Hyperparameters:
    ...
    # trick: loop-prelude-norm — looped transformer hyperparameters
    num_loop_layers = int(os.environ.get("NUM_LOOP_LAYERS", 3))  # trick: loop-prelude-norm
    num_loop_repeats = int(os.environ.get("NUM_LOOP_REPEATS", 3))  # trick: loop-prelude-norm
    loop_prelude_norm = bool(int(os.environ.get("LOOP_PRELUDE_NORM", "1")))  # trick: loop-prelude-norm — enable prelude normalization
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,loop_inject_scale",  # trick: loop-prelude-norm — added loop_inject_scale
    ).split(",")
    if pattern
)
...
class GPT(nn.Module):
    def __init__(
        ...
        rope_base: float,
        qk_gain_init: float,
        num_loop_layers: int,  # trick: loop-prelude-norm
        num_loop_repeats: int,  # trick: loop-prelude-norm
        loop_prelude_norm: bool,  # trick: loop-prelude-norm
    ):
        super().__init__()
        ...
        # trick: loop-prelude-norm — pure looped transformer: shared block repeated, no U-Net, no skip connections
        self.num_loop_layers = num_loop_layers  # trick: loop-prelude-norm
        self.num_loop_repeats = num_loop_repeats  # trick: loop-prelude-norm
        self.loop_prelude_norm_enabled = loop_prelude_norm  # trick: loop-prelude-norm

        # trick: loop-prelude-norm — re-inject normalized input at each loop iteration
        if loop_prelude_norm:
            self.prelude_norm = RMSNorm()  # trick: loop-prelude-norm — normalize x0 for re-injection
            self.loop_inject_scale = nn.Parameter(torch.zeros(model_dim, dtype=torch.float32))  # trick: loop-prelude-norm — zero-init: no injection at start
        else:
            self.prelude_norm = None  # trick: loop-prelude-norm
            self.loop_inject_scale = None  # trick: loop-prelude-norm

        self.loop_blocks = nn.ModuleList([  # trick: loop-prelude-norm
            Block(
                model_dim,
                num_heads,
                num_kv_heads,
        ...
        # No skip_weights — pure loop has no U-Net skip connections  # trick: loop-prelude-norm
        self.num_skip_weights = 0  # trick: loop-prelude-norm
        self.skip_weights = nn.Parameter(torch.empty(0, model_dim, dtype=torch.float32))  # trick: loop-prelude-norm — dummy, no skips
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        # trick: loop-prelude-norm — looped forward with input re-injection after each iteration
        for rep in range(self.num_loop_repeats):
            for block in self.loop_blocks:
                x = block(x, x0)
            # trick: loop-prelude-norm — re-inject normalized input after each full loop iteration
            if self.prelude_norm is not None:
                x = x + self.loop_inject_scale.to(dtype=x.dtype)[None, None, :] * self.prelude_norm(x0)  # trick: loop-prelude-norm
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        num_layers=args.num_layers,  # kept for compatibility but not used for block count  # trick: loop-prelude-norm
        ...
        num_loop_layers=args.num_loop_layers,  # trick: loop-prelude-norm
        num_loop_repeats=args.num_loop_repeats,  # trick: loop-prelude-norm
        loop_prelude_norm=args.loop_prelude_norm,  # trick: loop-prelude-norm
    ).to(device).bfloat16()
    ...
    block_named_params = list(base_model.loop_blocks.named_parameters())  # trick: loop-prelude-norm — collect params from loop blocks only
    matrix_params = [
        p
    ...
    # trick: loop-prelude-norm — add loop_inject_scale to scalar optimizer group
    if base_model.loop_inject_scale is not None:
        scalar_params.append(base_model.loop_inject_scale)  # trick: loop-prelude-norm
    # trick: loop-prelude-norm — skip_weights is empty, no need to add
    ...
    log0(f"trick:loop-prelude-norm loop_layers:{args.num_loop_layers} loop_repeats:{args.num_loop_repeats} effective_depth:{args.num_loop_layers * args.num_loop_repeats} prelude_norm:{args.loop_prelude_norm}")  # trick: loop-prelude-norm
