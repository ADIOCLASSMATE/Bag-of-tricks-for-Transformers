"""
Naive architecture GPT with strong training recipe + per-dimension smear gate.

Architecture is standard (no tricks) plus smear-gate: GQA, RoPE, RMSNorm (no learnable weight), QK-Norm, Tied Embeddings,
CastedLinear, restore_low_dim_params_to_fp32.

Smear-gate trick: per-dimension near-identity gate that mixes the current token's embedding with
the previous token's embedding. Each dimension has an independent learnable gate initialized to
sigmoid(3.0) ~ 0.95, so the model starts near-identity (mostly current token, small previous token).
...
class GPT(nn.Module):
    def __init__(
        ...
        # trick: smear-gate — per-dimension near-identity gate
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        smear_gate = torch.sigmoid(self.smear_gate).to(dtype=x.dtype)[None, None, :]
        x = smear_gate * x + (1.0 - smear_gate) * prev_x
...
def main() -> None:
    ...
    # trick: smear-gate — add smear_gate param to scalar optimizer
