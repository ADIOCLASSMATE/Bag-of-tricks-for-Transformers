...
class Hyperparameters:
    ...
    # trick: loop-depth-sampling — looped transformer + depth sampling hyperparameters
    num_loop_layers = int(os.environ.get("NUM_LOOP_LAYERS", 3))  # trick: loop-depth-sampling
    num_loop_repeats = int(os.environ.get("NUM_LOOP_REPEATS", 3))  # trick: loop-depth-sampling — max loop repeats (also used for eval)
    loop_depth_sampling = bool(int(os.environ.get("LOOP_DEPTH_SAMPLING", "1")))  # trick: loop-depth-sampling — enable depth sampling
    loop_depth_lambda = float(os.environ.get("LOOP_DEPTH_LAMBDA", "3.0"))  # trick: loop-depth-sampling — Poisson lambda
    loop_depth_min = int(os.environ.get("LOOP_DEPTH_MIN", "1"))  # trick: loop-depth-sampling — minimum loop repeats
    ...
    with torch.inference_mode():
        for batch_seq_start in range(seq_start, seq_end, local_batch_seqs):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                batch_loss = model(x, y).detach()  # trick: loop-depth-sampling — eval uses default num_loop_repeats (max depth)
...
class GPT(nn.Module):
    def __init__(
        ...
        rope_base: float,
        qk_gain_init: float,
        num_loop_layers: int,  # trick: loop-depth-sampling
        num_loop_repeats: int,  # trick: loop-depth-sampling
        loop_depth_sampling: bool,  # trick: loop-depth-sampling
    ):
        super().__init__()
        ...
        # trick: loop-depth-sampling — pure looped transformer: shared block repeated, no U-Net, no skip connections
        self.num_loop_layers = num_loop_layers  # trick: loop-depth-sampling
        self.num_loop_repeats = num_loop_repeats  # trick: loop-depth-sampling
        self.loop_depth_sampling = loop_depth_sampling  # trick: loop-depth-sampling

        self.loop_blocks = nn.ModuleList([  # trick: loop-depth-sampling
            Block(
                model_dim,
                num_heads,
                num_kv_heads,
        ...
        # No skip_weights — pure loop has no U-Net skip connections  # trick: loop-depth-sampling
        self.num_skip_weights = 0  # trick: loop-depth-sampling
        self.skip_weights = nn.Parameter(torch.empty(0, model_dim, dtype=torch.float32))  # trick: loop-depth-sampling — dummy, no skips
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, num_repeats: int | None = None) -> Tensor:  # trick: loop-depth-sampling — added num_repeats parameter
        x = self.tok_emb(input_ids)
        x = F.rms_norm(x, (x.size(-1),))
        x0 = x
        h = x

        # trick: loop-depth-sampling — use provided depth or default to max
        repeats = num_repeats if num_repeats is not None else self.num_loop_repeats  # trick: loop-depth-sampling

        # trick: loop-depth-sampling — pure looped forward pass, no U-Net skip connections
        for rep in range(repeats):  # trick: loop-depth-sampling
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        num_layers=args.num_layers,  # kept for compatibility but not used for block count  # trick: loop-depth-sampling
        ...
        num_loop_layers=args.num_loop_layers,  # trick: loop-depth-sampling
        num_loop_repeats=args.num_loop_repeats,  # trick: loop-depth-sampling
        loop_depth_sampling=args.loop_depth_sampling,  # trick: loop-depth-sampling
    ).to(device).bfloat16()
    ...
    block_named_params = list(base_model.loop_blocks.named_parameters())  # trick: loop-depth-sampling — collect params from loop blocks only
    matrix_params = [
        p
    ...
    # trick: loop-depth-sampling — skip_weights is empty, no need to add
    ...
    log0(f"trick:loop-depth-sampling loop_layers:{args.num_loop_layers} loop_repeats:{args.num_loop_repeats} effective_depth:{args.num_loop_layers * args.num_loop_repeats} depth_sampling:{args.loop_depth_sampling} depth_lambda:{args.loop_depth_lambda} depth_min:{args.loop_depth_min}")  # trick: loop-depth-sampling
    ...
    if args.warmup_steps > 0:
        ...
        for warmup_step in range(args.warmup_steps):
            ...
            for micro_step in range(grad_accum_steps):
                ...
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                    warmup_loss = model(x, y)  # trick: loop-depth-sampling — warmup uses default depth
    ...
    while True:
        ...
        # trick: loop-depth-sampling — sample loop depth per training step (per micro-batch)
        if args.loop_depth_sampling:  # trick: loop-depth-sampling
            sampled_depth = int(torch.clamp(  # trick: loop-depth-sampling
                torch.poisson(torch.tensor(args.loop_depth_lambda, dtype=torch.float32, device=device)),  # trick: loop-depth-sampling
                min=args.loop_depth_min, max=args.num_loop_repeats  # trick: loop-depth-sampling
            ).item())  # trick: loop-depth-sampling
        else:  # trick: loop-depth-sampling
            sampled_depth = args.num_loop_repeats  # trick: loop-depth-sampling — fixed depth when sampling disabled
        ...
        for micro_step in range(grad_accum_steps):
            ...
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y, num_repeats=sampled_depth)  # trick: loop-depth-sampling — pass sampled depth
        ...
        if should_log_train:
            log0(
                ...
                f"sampled_depth:{sampled_depth}"  # trick: loop-depth-sampling — log sampled depth
            )
            wandb_log(
                {
                    ...
                    "train/sampled_depth": sampled_depth,  # trick: loop-depth-sampling
                },
                step=step,
            )
