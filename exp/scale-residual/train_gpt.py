"""
Baseline GPT + learnable attn_scale/mlp_scale ablation.

Same as baseline (naive architecture + strong training recipe), but adds
learnable per-dimension scaling on attention and MLP residual connections.

trick: scale-residual — adds attn_scale and mlp_scale parameters (ones(dim))
so residual connections become: x = x + scale * sublayer(norm(x)) instead of
the standard x = x + sublayer(norm(x)).
"""
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn_scale = nn.Parameter(torch.ones(dim, dtype=torch.float32))  # trick: scale-residual
        self.mlp_scale = nn.Parameter(torch.ones(dim, dtype=torch.float32))  # trick: scale-residual

    def forward(self, x: Tensor) -> Tensor:
        x = x + self.attn_scale.to(dtype=x.dtype)[None, None, :] * self.attn(self.attn_norm(x))  # trick: scale-residual
        x = x + self.mlp_scale.to(dtype=x.dtype)[None, None, :] * self.mlp(self.mlp_norm(x))  # trick: scale-residual
        return x
