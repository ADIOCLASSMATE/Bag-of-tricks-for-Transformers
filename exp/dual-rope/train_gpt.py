"""
Naive architecture GPT with strong training recipe + Dual RoPE.

Trick: Per-layer RoPE config -- "global" layers use theta=1M + partial_rotary=0.25;
"sliding" layers use standard RoPE. ONLY changes RoPE parameters per layer,
does NOT change the attention pattern itself.
...
class Rotary(nn.Module):
    def __init__(self, dim: int, base: float = 10000.0, partial_rotary_factor: float = 1.0):
        ...
        if self.rope_dim % 2 != 0:
            self.rope_dim -= 1
        inv_freq = 1.0 / (base ** (torch.arange(0, self.rope_dim, 2, dtype=torch.float32) / dim))
...
def apply_rotary_emb(x: Tensor, cos: Tensor, sin: Tensor, rope_dim: int | None = None) -> Tensor:
    ...
    # Partial rotation: rotate first rope_dim dims, pass through the rest unchanged
    half = rope_dim // 2
    x_rot = x[..., :rope_dim]
    x_pass = x[..., rope_dim:]
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        rotary_base = rope_theta if rope_theta is not None else rope_base
        self.rotary = Rotary(self.head_dim, base=rotary_base, partial_rotary_factor=partial_rotary_factor)
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, rope_theta=rope_theta, partial_rotary_factor=partial_rotary_factor)
...
class GPT(nn.Module):
    def __init__(
        ...
        # Determine layer types: every Nth layer is "global", rest are "sliding", last is always "global"
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    mlp_mult,
                    rope_base,
                    rope_theta=global_rope_theta if layer_types[i] == "global" else None,
                    partial_rotary_factor=partial_rotary_factor if layer_types[i] == "global" else 1.0,
                )
                for i in range(num_layers)
            ]
        )
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        global_rope_theta=args.global_rope_theta,
