"""
Baseline GPT + small embedding init ablation.

Same as baseline (naive architecture + strong training recipe), but initializes
the tied embedding with a small standard deviation (0.005 by default) instead
of PyTorch's default nn.Embedding init.

trick: small-embed-init — uses nn.init.normal_(tok_emb.weight, std=0.005)
when tie_embeddings is True. This stabilizes early training by keeping initial
logit magnitudes small, preventing extreme gradients at the output layer.
"""
...
class Hyperparameters:
    ...
    tied_embed_init_std = float(os.environ.get("TIED_EMBED_INIT_STD", 0.005))  # trick: small-embed-init
...
class GPT(nn.Module):
    def __init__(
        ...
        tie_embeddings: bool,
        rope_base: float,
        tied_embed_init_std: float,  # trick: small-embed-init
    ):
        super().__init__()
        self.tie_embeddings = tie_embeddings
        self.tied_embed_init_std = tied_embed_init_std  # trick: small-embed-init
        ...
        self._init_weights()  # trick: small-embed-init

    def _init_weights(self) -> None:  # trick: small-embed-init
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        tied_embed_init_std=args.tied_embed_init_std,  # trick: small-embed-init
    ).to(device).bfloat16()
