"""
GPT with ln-scale trick: depth-dependent RMSNorm output scaling.

Each Block scales its RMSNorm outputs by 1/sqrt(layer_idx+1) before feeding
into the attention and MLP sublayers. This counters the variance growth that
accumulates through residual connections in deeper models. All other
architecture and training recipe matches the baseline exactly.
"""
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x = x + self.attn(self.attn_norm(x) * s)
        x = x + self.mlp(self.mlp_norm(x) * s)
        return x
