"""
GPT with attention-head gating trick.

Baseline architecture + a learned per-head sigmoid gate in CausalSelfAttention:
  - A small linear layer (attn_gate) maps the first 12 channels of the input to
    a per-head gate value, initialized to zero so the gate starts at sigmoid(0)=0.5.
  - After SDPA, the output y is multiplied by the gate, allowing the model to
    learn to shut off individual attention heads.

All other architecture and training recipe matches the baseline exactly.
"""
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        # trick: attention-head gating — learned per-head sigmoid gate
        self.attn_gate_channels = 12
        self.attn_gate = CastedLinear(self.attn_gate_channels, num_heads, bias=False)
