...
| Weight decay family | 训练调度 | nanogpt-speedrun, parameter-golf, slowrun | nanogpt `2025-02-08_WeightDecay`, `2025-11-10_CautiousWD`, `2025-12-18_CautiousWDAdam`; parameter-golf `T06`; slowrun `T05`, `T07` | 调整 WD 数值、把 cautious WD 扩展到不同参数组，或把 WD 做成显式多阶段 schedule。 |
| Warmdown / cooldown | 训练调度 | nanogpt-speedrun, parameter-golf, slowrun | nanogpt `2025-03-06_LongerCooldown`; parameter-golf `T02`; slowrun `T04` | 改训练后段的学习率衰减长度或比例，本质上是在拉长或重调 late-stage decay。 |
| SWA / EMA | 训练调度 | parameter-golf, slowrun | parameter-golf `T09`, `T10`; slowrun `T06` | 用 SWA 或 EMA 维护平滑权重；slowrun 文档里明确包含 final-vs-EMA / blend 候选比较。 |
...
| PerEpochReshuffle | 数据 | slowrun | `T01 PerEpochReshuffle` | 每个 epoch 重新打乱预分词后的样本顺序，改变训练样本呈现次序。 |
| BosAlign | 数据 | nanogpt-speedrun | `track_1_short / 2025-07-12_BosAlign` | 把训练数据切片起点对齐到 BOS token，减少跨文档切分带来的上下文边界噪声。 |
| DecoderReplayEnable | 数据/训练 | slowrun | `T02 DecoderReplayEnable` | 在训练后段重放 decoder span，让模型重复见到部分片段；不同 track 的具体 replay 细节略有差异。 |
| ReplayScheduleTuning | 训练调度 | slowrun | `T03 ReplayScheduleTuning` | 进一步调 replay 的开始时机、循环次数和 span，属于 replay 机制上的调度细化。 |
| U-Net / skip family | 结构 | nanogpt-speedrun, slowrun | nanogpt `2025-11-18_RefineSkip`; slowrun `A03` | 在层间加入 encoder-decoder 式镜像 skip；nanogpt 版本进一步把 skip 结构从大 U 收紧到更精简的连接。 |
| Value path family | 结构 | nanogpt-speedrun, parameter-golf, slowrun | nanogpt `2024-12-04_ValueEmbed`, `2025-04-22_Record8`, `2025-08-28_Medium_NewValemb`, `2025-10-04_GPT2MediumLayerReuse`, `2026-01-26-UntieValueEmbeddings`; parameter-golf `A10`; slowrun `A01` | 给网络增加额外的 value 注入路径：包括多层 VE、x0 投影、layer reuse，以及 tied/untied 的 value embedding 变体。 |
...
| SwiGLU | 结构 | slowrun | `A02 SwiGLU` | 把 ReLU^2 MLP 换成带门控的 SwiGLU MLP，用 gated product 重写 FFN 路径。 |
...
| HalfTruncatedRoPE | 结构 | slowrun | `A06 HalfTruncatedRoPE` | 只旋转一半的 head 维度，另一半保持静止；在 slowrun 中与 key-offset 改动同属一组 bundle。 |
| PartialKeyOffset | 结构 | nanogpt-speedrun, slowrun | nanogpt `2025-12-14_PartialKeyOffset`; slowrun `A07` | 对长窗口层的部分 key 维度做前移/offset，改变局部注意力中的 key 对齐方式。 |
| AttentionHeadGating | 结构 | slowrun | `A04 AttentionHeadGating` | 在 attention 输出端加入 per-head sigmoid gate，让每个头的贡献可学习缩放。 |
...
| XSA family | 结构 | parameter-golf, slowrun | parameter-golf `A06`, `A07`; slowrun `A05` | XSA 会移除 attention 输出中的 self-value 分量；parameter-golf 进一步区分只加在后 4 层或全层。 |
...
| ResidualLambdaInit11 | 初始化 | slowrun | `T08 ResidualLambdaInit11` | 把 residual lambda 初值从 1.0 调到 1.1，是对残差分支初始强度的小幅改动。 |
